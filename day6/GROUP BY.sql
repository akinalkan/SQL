--GROUP BY -
/*
GROUP BY cumlesi bir select ifadesin de satirlari, 
sutunlari degerlerine gore ozet olarak gruplamak icin kullanilir.

GROUP BY cmlesi her grup basina satir dondurur.

GROUP BY genel de AVG(), COUNT(), MAX(),MIN() ve  SUM() 
gibi aggregate fonksiyonlari ile bbirlikte kullanilir
*/
...
select urun_adi,
min(urun_miktari)as min_meyve,
max(urun_miktari)as max_meyve from manav
where urun_adi is not null
group by urun_adi order by max_meyve desc ;
...
select cinsiyet,round(avg(maas)) as ort_maas
from personel
group cinsiyet;
...
select ulke ,cinsiyet , round(avg(yas)) as yas_ort , count(*)
from personel
where maas>30000						
group by ulke , cinsiyet 
