...
from kelimeler;
  ...
  select * 
  from kelimeler
  where kelime SIMILAR TO '%(at|ot)%'
  
  --2.yol[like]
  select *
  from kelimeler
  where kelime ~~*'%at%' or kelime ~~*'%ot%';
  
--3.Yol[like]
select * 
from kelimeler
where kelime ILIKE '%at%' or kelime ILIKE '%ot%';

--REGEX
select * 
from kelimeler
where kelime ~* 'ot' or kelime ~* 'at';
...
select * 
from kelimeler 
where kelime ILIKE 'ho%' or kelime ILIKE'hi%';
...
select * 
from kelimeler 
where kelime SIMILAR TO 'h[0-9,A-Z,a-z]t';
...
select * 
from kelimeler
where kelime SIMILAR TO 'h[a-e](.*)%t';

--REGEX ile
select * 
from kelimeler 
where kelime ~* 'h[a-e](.*)t';

--İlk karakteri 's', 'a' veya 'y' olan "kelime" değerlerini çağırın.
select kelime
from kelimeler
where kelime ~*'^[say](.*)';--> ^ baslangic'i temsil eder
...
select *
from kelimeler
where kelime SIMILAR TO 'h[a|i]t';

--REGEX ile
select * 
from kelimeler 
where kelime ~ '^h[a|i]t$';
...
select*
from kelimeler
where kelime similar to '[b-s]_l%';

--içerisinde en az 2 adet o o barıdıran kelimelerin tüm bilgilerini listeleyiniz.

select*
from kelimeler
where kelime similar to '%[o][o]%';

select*
from kelimeler 
where kelime  similar to '%[o]{2}%'; -->{...} icine yazilan rakam bir onceki[...] icine yazilan
									--karakterin kac tane oldugunu belirtir.
